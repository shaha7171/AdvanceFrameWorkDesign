...
public class ActionEngine {
	...
	public boolean isElementPresent_custom(WebElement element,String fieldName){
		...
		try {
			...
			extent().log(Status.FAIL, "Checking for presence of field: " +fieldName +" not tested due to exception: "+e);
			return flag;
		}
	}
	...
	public void selectDropDownByVisibleText_custom(WebElement element, String fieldName, String ddVisibleText) throws Throwable {
		try {
			...
			extent().log(Status.FAIL, "Dropdown value not selected for field: " +fieldName +"  due to exception: "+e);
		}
	}
	...
	public void assertEqualsString_custom(String expvalue, String actualValue, String locatorName) throws Throwable {
		try {
			if(actualValue.equals(expvalue)) {
				extent().log(Status.PASS, "String Assertion is successful on field "+ locatorName + " Expected value was: "+ expvalue + " actual value is: "+actualValue);
			}else {
				extent().log(Status.FAIL, "String Assertion FAILED on field "+ locatorName + " Expected value was: "+ expvalue + " actual value is: "+actualValue);
	...
	//Fluent Wait
		...
		Wait<WebDriver> fwait = new FluentWait<WebDriver>(DriverFactory.getInstance().getDriver())
			       ...
			       .withMessage("User defined Timed out after 20 seconds")
...
Throws: NoSuchFrameException - If the frame is not found*/
		...
		public void switchToFrame(String frameNameorID) {
			try {
				...
				extent().log(Status.FAIL, "Unable to locate frame with Name or ID " + frameNameorID + " => " + e.getStackTrace());
			} catch (Exception e) {
				extent().log(Status.FAIL, "Unable to locate frame with Name or ID  " + frameNameorID + " => " + e.getStackTrace());
			}
		}
...
Throws: NoSuchFrameException - If the given element is neither an iframe nor a frame element. And StaleElementReferenceException - If the WebElement has gone stale.*/
		...
		public void switchToFrame(WebElement frameElement) {
			try {
				if (isElementPresent_custom(frameElement,"verifying presence of frame")) {
					driver().switchTo().frame(frameElement);
					
					extent().log(Status.PASS, "Navigated to frame with element "+ frameElement);
				} else {
					extent().log(Status.FAIL, "Frame is displayed but unable to navigate to frame with element "+ frameElement);
				}
			} catch (NoSuchFrameException e) {
				extent().log(Status.FAIL, "Unable to locate frame with element " + frameElement + e.getStackTrace());
			} catch (StaleElementReferenceException e) {
				extent().log(Status.FAIL, "Element with " + frameElement + "is not attached to the page document" + e.getStackTrace());
			} catch (Exception e) {
				extent().log(Status.FAIL, "Unable to navigate to frame with element " + frameElement + e.getStackTrace());
			}
		}
		
		/*Some times when there are multiple Frames (Frame in side a frame), we need to first switch to the parent frame and then we need to switch to the child frame.*/
		...
		public void switchToFrame(String ParentFrameNameorID, String ChildFrameNameorID) {
			try {
				...
				extent().log(Status.PASS, "Navigated to innerframe with id " + ChildFrameNameorID + "which is present on frame with id" + ParentFrameNameorID);
			} catch (NoSuchFrameException e) {
				extent().log(Status.FAIL, "Unable to locate frame with id " + ParentFrameNameorID + " or " + ChildFrameNameorID + e.getStackTrace());
			} catch (Exception e) {
				extent().log(Status.FAIL, "Unable to navigate to innerframe with id " + ChildFrameNameorID + "which is present on frame with id" + ParentFrameNameorID + e.getStackTrace());
			}
		}
		
		/*After working with the frames, main important is to come back to the web page. if we don't switch back to the default page, driver will throw an exception. */
		...
		public int NumberOfFrames(By tagName) {
			extent().info("Finding total Number of frames on the page");
			List<WebElement> frames =driver().findElements(tagName);
			return frames.size();
		...
		public void closeAllOtherWindows(String mainwindow) {
			...
			if (driver().getWindowHandles().size() == 1) {
				...
				extent().log(Status.FAIL, "Switched to the main window but mismatching the size of the total windows => " +title);
			}
		}
